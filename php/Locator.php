...
class Locatr {
  ...
  private static function getFinalUrl() {
    error_log("URL: " . BASE_API_URL . $_REQUEST["action"] . "/" . DATA_OUTPUT_TYPE . "?" . $_SERVER['QUERY_STRING'], 0);    
    return html_entity_decode(BASE_API_URL . $_REQUEST["action"] . "/" . DATA_OUTPUT_TYPE . "?" . $_SERVER['QUERY_STRING']);
    //return html_entity_decode("https://maps.googleapis.com/maps/api/place/search/json?location=22.2855200,114.1576900&action=nearbysearch&radius=1000&sensor=false&key=AIzaSyDlioxIusIXAs2jnbToEmf0O1pBFlP7mkQ&types=restaurant&page=1&start=0&limit=25");
